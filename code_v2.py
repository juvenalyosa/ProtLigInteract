...
ATOM_DEFS = {
    ...
    'protein_h_donors': {
        'main': ['N'], 'LYS': ['NZ'], 'ARG': ['NE', 'NH1', 'NH2'], 'HIS': ['ND1', 'NE2'],
        'TRP': ['NE1'], 'GLN': ['NE2'], 'ASN': ['ND2'], 'SER': ['OG'], 'THR': ['OG1'], 'TYR': ['OH']
    },
    'protein_h_acceptors': {
        'main': ['O'], 'ASP': ['OD1', 'OD2'], 'GLU': ['OE1', 'OE2'], 'HIS': ['ND1', 'NE2'],
...
class ProtLigInteractDialog(QtWidgets.QDialog):
    ...
    def _on_load_pdb(self):
        ...
        if ligand:
            ...
            # persist last ligand
            try:
                self._settings.update({'last_ligand_chain': ligand[0], 'last_ligand_resi': int(ligand[1]), 'last_ligand_resn': ligand[2]})
                self._save_settings()
            except Exception:
                pass
    ...
    def _on_choose_ligand(self):
        ...
        if not items:
            QtWidgets.QMessageBox.information(self, 'No ligands', 'No non-polymer, non-water, non-metal residues detected.')
            return
        ...
        def do_select():
            ...
            it = next((x for x in items_full if str(x['chain'])==chain and int(x['resi'])==resi and str(x['resn'])==resn), None)
            if it is None:
            ...
            try:
                self._settings.update({'last_ligand_chain': it['chain'], 'last_ligand_resi': int(it['resi']), 'last_ligand_resn': it['resn']})
                self._save_settings()
            except Exception:
                pass
            dlg.accept()
        ...
        def _save_chooser_settings():
            try:
                self._settings.update({
                    ...
                    'chooser_ref_chain': chain_combo.currentText() if chain_combo.isEnabled() and chain_combo.count()>0 else '',
    ...
    def _on_calc_interactions(self):
        ...
        try:
            self._settings.update({'last_ligand_chain': self.ligand_info[0], 'last_ligand_resi': int(self.ligand_info[1]), 'last_ligand_resn': self.ligand_info[2]})
            self._save_settings()
        except Exception:
            pass
    ...
    def _precompute_atom_features(self):
        ...
        for atom in self.structure.get_atoms():
            ...
            atom_info = {'atom': atom, 'resn': resn, 'resi': resi, 'chain': chain.id,
                         'coord': atom.coord, 'name': atom.name, 'element': atom.element,
                         'origin': 'ligand' if is_ligand else ('protein' if is_aa(res) else 'other')}

            if is_ligand:
                features['ligand_atoms'].append(atom_info)
                if atom.element in ATOM_DEFS['halogens']: features['ligand_halogens'].append(atom_info)
                if atom.element in ['O', 'N']: features['ligand_h_donors_acceptors'].append(atom_info)
                if atom.element == 'C': features['ligand_hydrophobic'].append(atom_info)
                if atom.element in ['O', 'N', 'S']: features['ligand_metal_acceptors'].append(atom_info)
            elif is_aa(res):
                features['protein_atoms'].append(atom_info)
                if resn in ATOM_DEFS['protein_positive'] and atom.name in ATOM_DEFS['protein_positive'][resn]: features['protein_positive'].append(atom_info)
                if resn in ATOM_DEFS['protein_negative'] and atom.name in ATOM_DEFS['protein_negative'][resn]: features['protein_negative'].append(atom_info)
                if (resn in ATOM_DEFS['protein_h_donors'] and atom.name in ATOM_DEFS['protein_h_donors'][resn]) or (atom.name == 'N' and resn != 'PRO'): features['protein_h_donors'].append(atom_info)
                if (resn in ATOM_DEFS['protein_h_acceptors'] and atom.name in ATOM_DEFS['protein_h_acceptors'][resn]) or (atom.name == 'O'): features['protein_h_acceptors'].append(atom_info)
                if resn in ATOM_DEFS['protein_hydrophobic_res'] and atom.element == 'C': features['protein_hydrophobic'].append(atom_info)
    ...
    def _detect_ligand_rings(self, features):
        ...
        for cyc in cycles:
            ...
            if aro_frac < 0.8:
                continue
            radius = get_ring_radius(ring_coords, centroid)
            rings.append({'resn': lig_resn, 'resi': lig_resi, 'chain': lig_chain, 'coords': ring_coords,
                          'centroid': centroid, 'normal': normal, 'radius': radius})
        return rings
    ...
    def _calculate_all_interactions(self):
        ...
        def is_hbond(donor, acceptor):
            ...
            if Hs:
                best = max(angle_between_vectors(h['coord'] - donor['coord'], acceptor['coord'] - h['coord']) for h in Hs)
        ...
        grid_lig_oxyn = self._NeighborGrid([a for a in features['ligand_atoms'] if a['element'] in ('O','N')], cell_size=sb_cut)
        ...
        grid_lig_posN = self._NeighborGrid([a for a in features['ligand_atoms'] if a['element']=='N'], cell_size=cp_cut)
        ...
        for l_ring in features['ligand_rings']:
            ...
            for p_pos in grid_p_pos.query(l_ring['centroid'], cp_cut):
                ...
                if d <= GEOMETRY_CRITERIA['cation_pi_dist'] and angle <= GEOMETRY_CRITERIA['cation_pi_angle']:
                    add_interaction('Cation-Pi', p_pos, l_placeholder, d, f"Angle: {angle:.1f}°", extra={'ligand_ring': l_ring})
            # ring-ring
            ...
            for p_ring in grid_p_rings.query(l_ring['centroid'], GEOMETRY_CRITERIA['pi_t_dist']):
                ...
                if d <= GEOMETRY_CRITERIA['pi_pi_dist'] and (angle <= GEOMETRY_CRITERIA['pi_pi_angle'] or angle >= 180 - GEOMETRY_CRITERIA['pi_pi_angle']):
                    add_interaction('Pi-Pi Stacking', p_placeholder, l_placeholder, d, f"Angle: {angle:.1f}°", extra={'prot_ring': p_ring, 'ligand_ring': l_ring})
                elif GEOMETRY_CRITERIA['pi_pi_dist'] < d <= GEOMETRY_CRITERIA['pi_t_dist'] and GEOMETRY_CRITERIA['pi_t_angle_low'] <= angle <= GEOMETRY_CRITERIA['pi_t_angle_high']:
                    add_interaction('T-Shaped Pi-Pi', p_placeholder, l_placeholder, d, f"Angle: {angle:.1f}°", extra={'prot_ring': p_ring, 'ligand_ring': l_ring})
        ...
        for l_atom in features['ligand_atoms']:
            ...
            for p_atom in neighbors:
                ...
                if d > max_r:
                    continue
                is_hydrophobic = (
                    p_atom['resn'] in ATOM_DEFS['protein_hydrophobic_res'] and p_atom['element'] == 'C' and l_atom['element'] == 'C' and d <= GEOMETRY_CRITERIA['hydrophobic_dist']
                )
        ...
        for (chain, resi), data in vdw_contacts.items():
            ...
            if any(i for i in interactions if i['prot_res'] == f"{resn} {chain}{resi}" and i['type'] != 'Van der Waals'):
                continue
            interactions.append({
                'type': 'Van der Waals',
                'prot_res': f"{resn} {chain}{resi}",
        ...
        def add_interaction(inter_type, prot_atom, lig_atom, dist, details=""):
            ...
            if pair_key in found_pairs: return
            found_pairs.add(pair_key)
            interactions.append({'type': inter_type, 'prot_res': f"{prot_atom['resn']} {prot_atom['chain']}{prot_atom['resi']}",
                                 'prot_atom': prot_atom['name'], 'lig_atom': lig_atom['name'],
                                 'distance': dist, 'details': details})
        ...
        for p_don in features['protein_h_donors']:
            for l_acc in features['ligand_h_donors_acceptors']:
                ...
                if dist <= GEOMETRY_CRITERIA['h_bond_dist']: add_interaction('Hydrogen Bond', p_don, l_acc, dist)
        for l_don in features['ligand_h_donors_acceptors']:
            for p_acc in features['protein_h_acceptors']:
                ...
                if dist <= GEOMETRY_CRITERIA['h_bond_dist']: add_interaction('Hydrogen Bond', p_acc, l_don, dist)

        for p_pos in features['protein_positive']:
            for l_neg in features['ligand_atoms']:
                if l_neg['element'] in ['O', 'N']:
                    dist = distance(p_pos['coord'], l_neg['coord'])
                    if dist <= GEOMETRY_CRITERIA['salt_bridge_dist']: add_interaction('Salt Bridge', p_pos, l_neg, dist)
        for p_neg in features['protein_negative']:
            for l_pos in features['ligand_atoms']:
                if l_pos['element'] == 'N':
                    dist = distance(p_neg['coord'], l_pos['coord'])
                    if dist <= GEOMETRY_CRITERIA['salt_bridge_dist']: add_interaction('Salt Bridge', p_neg, l_pos, dist)

        for metal in features['metals']:
            for l_acc in features['ligand_metal_acceptors']:
                dist = distance(metal['coord'], l_acc['coord'])
                if dist <= GEOMETRY_CRITERIA['metal_coordination_dist']: add_interaction('Metal Coordination', metal, l_acc, dist)

        for l_hal in features['ligand_halogens']:
            for p_acc in features['protein_h_acceptors']:
                dist = distance(l_hal['coord'], p_acc['coord'])
                if dist <= GEOMETRY_CRITERIA['halogen_dist']: add_interaction('Halogen Bond', p_acc, l_hal, dist)
        ...
        for p_ring in features['protein_rings']:
            ...
            for l_pos in features['ligand_atoms']:
                if l_pos['element'] == 'N':
                    ...
                    if dist <= GEOMETRY_CRITERIA['cation_pi_dist']: add_interaction('Cation-Pi', p_placeholder, l_pos, dist)
        ...
        for l_ring in features['ligand_rings']:
            ...
            for p_ring in features['protein_rings']:
                ...
                angle = angle_between_vectors(p_ring['normal'], l_ring['normal'])
                if dist <= GEOMETRY_CRITERIA['pi_pi_dist'] and (angle <= GEOMETRY_CRITERIA['pi_pi_angle'] or angle >= 180 - GEOMETRY_CRITERIA['pi_pi_angle']):
                    add_interaction('Pi-Pi Stacking', p_placeholder, l_placeholder, dist, f"Angle: {angle:.1f}°")
                elif GEOMETRY_CRITERIA['pi_pi_dist'] < dist <= GEOMETRY_CRITERIA['pi_t_dist'] and GEOMETRY_CRITERIA['pi_t_angle_low'] <= angle <= GEOMETRY_CRITERIA['pi_t_angle_high']:
        ...
        for p_atom in features['protein_atoms']:
            for l_atom in features['ligand_atoms']:
                ...
                is_hydrophobic_contact = (p_atom['resn'] in ATOM_DEFS['protein_hydrophobic_res'] and
                                          p_atom['element'] == 'C' and l_atom['element'] == 'C' and
                                          dist <= GEOMETRY_CRITERIA['hydrophobic_dist'])
        ...
        for (chain, resi), data in hydrophobic_contacts.items():
            resn = next(a['resn'] for a in features['protein_atoms'] if a['chain'] == chain and a['resi'] == resi)
            interactions.append({'type': 'Hydrophobic', 'prot_res': f"{resn} {chain}{resi}",
                                 'prot_atom': ','.join(sorted(data['prot_atoms'])),
                                 'lig_atom': ','.join(sorted(data['lig_atoms'])),
                                 'distance': data['min_dist'], 'details': 'Cluster'})

        for (chain, resi), data in vdw_contacts.items():
            resn = next(a['resn'] for a in features['protein_atoms'] if a['chain'] == chain and a['resi'] == resi)
            is_specific = any(i for i in interactions if i['prot_res'] == f"{resn} {chain}{resi}" and i['type'] != 'Hydrophobic')
            if not is_specific:
                 interactions.append({'type': 'Van der Waals', 'prot_res': f"{resn} {chain}{resi}",
                                     'prot_atom': ','.join(sorted(data['prot_atoms'])),
                                     'lig_atom': ','.join(sorted(data['lig_atoms'])),
                                     'distance': data['min_dist'], 'details': 'Cluster'})
    ...
    def _beautify_scene(self):
        ...
        if not self.loaded_object: return
    ...
    def _make_ring_disc(self, name, centroid, normal, radius, color_or_type):
        ...
        if isinstance(color_or_type, str) and color_or_type in COLOR_MAP:
            try:
                ov = self._settings.get('style_overrides', {}).get(color_or_type, {}) if hasattr(self, '_settings') else {}
    ...
    def _update_scale_bar_for_export(self):
        ...
        try:
            ...
            diag = 10.0
        right = np.array(view[0:3]); up = np.array(view[3:6])
    ...
    def _update_title_for_export(self):
        ...
        try:
            ...
            center = np.array([view[12], view[13], view[14]])
            diag = 10.0
        up = np.array(view[3:6]); up = up / (np.linalg.norm(up) or 1.0)
    ...
    def _on_show_hist(self):
        ...
        try:
            from PySide2.QtWidgets import QDialog, QVBoxLayout, QPlainTextEdit, QPushButton
        except Exception:
    ...
    def _on_edit_styles(self):
        ...
        table.setHorizontalHeaderLabels(['Type','Color','#Dash Radius','#Dash Length','#Dash Gap','Disc Thickness','Disc Opacity','Cloud Opacity'])
        ...
        for row, t in enumerate(types):
            ...
            for col, key, default in [(2,'dash_radius', STYLE_MAP.get(t,{}).get('dash_radius',0.08)),
            ...
            default_thick = float(ov.get('disc_thickness', self.disc_thickness_spin.value() if hasattr(self,'disc_thickness_spin') else 0.10))
        ...
        def on_save():
            ...
            for row, t in enumerate(types):
                ...
                rec = {'dash_radius': dr, 'dash_length': dl, 'dash_gap': dg, 'disc_thickness': thick, 'disc_opacity': disc, 'cloud_opacity': cloud}
    ...
    def _init_settings(self):
        ...
        if hasattr(self, 'legend_corner_combo'):
            ...
            combo.currentTextChanged.connect(lambda _: (self._update_legend(for_export=False, anchor=self._legend_anchor()), self._save_settings()))
        # Legend export
        if hasattr(self, 'legend_export_cb'):
        ...
        if hasattr(self, 'disc_thickness_spin'):
            ...
            self.disc_thickness_spin.valueChanged.connect(lambda _v: (self._refresh_ring_discs(), self._save_settings()))
        # Distance labels
    ...
    def _legend_anchor(self):
        txt = getattr(self, 'legend_corner_combo', None).currentText() if hasattr(self, 'legend_corner_combo') else 'Top Right'
        mapping = {
            'Top Right': 'top_right',
            'Top Left': 'top_left',
    ...
    def _refresh_ring_discs(self):
        ...
        try:
            for rid in list(self._selected_ids):
                ...
                if 'prot_ring' in inter:
                    r = inter['prot_ring']
                    self._make_ring_disc(f'intRingDiscProt_{rid}', r['centroid'], r['normal'], r.get('radius', 1.8), ring_color)
                    cmd.group(inter_type_group, f'intRingDiscProt_{rid}')
                if 'ligand_ring' in inter:
                    r = inter['ligand_ring']
                    self._make_ring_disc(f'intRingDiscLig_{rid}', r['centroid'], r['normal'], r.get('radius', 1.8), ring_color)
    ...
    def _collect_settings_from_ui(self):
        data = {}
        try:
            data['legend_corner'] = self.legend_corner_combo.currentText() if hasattr(self, 'legend_corner_combo') else 'Top Right'
            data['legend_export'] = bool(self.legend_export_cb.isChecked()) if hasattr(self, 'legend_export_cb') else True
            data['disc_thickness'] = float(self.disc_thickness_spin.value()) if hasattr(self, 'disc_thickness_spin') else 0.10
            ...
            # compute on load
            data['compute_on_load'] = bool(self.compute_on_load_cb.isChecked()) if hasattr(self, 'compute_on_load_cb') else False
            ...
            data['show_angle_labels'] = bool(self.show_angle_labels_cb.isChecked()) if hasattr(self, 'show_angle_labels_cb') else False
            data['include_scale'] = bool(self.include_scale_cb.isChecked()) if hasattr(self, 'include_scale_cb') else True
            data['scale_length'] = int(self.scale_length_spin.value()) if hasattr(self, 'scale_length_spin') else 10
            data['include_title'] = bool(self.include_title_cb.isChecked()) if hasattr(self, 'include_title_cb') else False
            data['title_text'] = self.title_line.text() if hasattr(self, 'title_line') else ''
            data['confirm_remove_all'] = bool(self._settings.get('confirm_remove_all', True)) if hasattr(self, '_settings') else True
        except Exception:
            pass
        return data
            ...
            for rid, inter in enumerate(self.interactions):
                ...
                for obj in (f'intCloudProt_{rid}', f'intCloudLig_{rid}'):
                    try:
                        ...
                        ov = self._settings.get('style_overrides', {}).get(itype, {}) if hasattr(self, '_settings') else {}
                ...
                for which, key, obj in (('prot','prot_ring', f'intRingDiscProt_{rid}'), ('lig','ligand_ring', f'intRingDiscLig_{rid}')):
                    try:
                        cmd.delete(obj)
                    except Exception:
                        pass
                ...
                try:
                    ...
                    if bool(self._settings.get('show_angle_labels', False)) if hasattr(self, '_settings') else False:
                        ...
                        if detail and inter.get('prot_atom') and inter.get('lig_atom'):
                            ...
                            pm = cmd.get_model(prot_res_sel); lm = cmd.get_model(lig_atom_sel)
                            if pm.atom and lm.atom:
                                p = np.array(pm.atom[0].coord); q = np.array(lm.atom[0].coord)
                                mid = (p + q) / 2.0
    ...
    def _on_clear(self):
        ...
        try:
            for rid in list(self._selected_ids):
                for prefix in ('intRes_', 'int_dash_', 'intCloudProt_', 'intCloudLig_', 'intRingDiscProt_', 'intRingDiscLig_'):
                    try:
    ...
    def _on_remove_all_visuals(self):
        ...
        try:
            ...
            for pat in ('intRes_*', 'int_dash_*', 'intCloudProt_*', 'intCloudLig_*', 'intRingDiscProt_*', 'intRingDiscLig_*', 'legend_*', 'angle_label_*', 'scale_label', 'title_label'):
                try:
                    cmd.delete(pat)
                except Exception:
                    pass
        ...
        try:
            ...
            with open(out, 'w', newline='') as f:
                ...
                for i in self.interactions:
                    w.writerow([i['type'], i['prot_res'], i['prot_atom'], i['lig_atom'], f"{i['distance']:.2f}", i.get('details', '')])
